...
import { useNavigate } from "react-router-dom"; // useNavigate imported from react-router

const CreatePost = () => {
  const { addPost } = useContext(PostList);
  const navigate = useNavigate(); //useNavigate hook can be used to do dynamic navigation programmatically. It returns a function that allows you to programmatically navigate to different routes in your application.
  ...
  const handleSubmit = (event) => {
    ...
    // .then(console.log);
    .then(postObj => {
      addPost(postObj); //sending post object to addPost
      navigate("/"); //2) so using here; This updates the URL in the browser to the root path ("/"). Since "/" is mapped to the <PostList /> component within the children of the root route, navigating to "/" will render the <PostList /> component inside the <App /> component.
    }); 
    // navigate("/"); //1) not using here as hosakta ha POST pura kaam na karpaye ya error ajaye and navigate use hojaye
  };
